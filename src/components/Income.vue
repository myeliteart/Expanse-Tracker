<template>
    <Form class="text-left mt-4" @submit="Validate.increment" :validation-schema="Validate.schema">
        <div>
            <label for="desc">Income Description</label>
        ...
        <!-- <div class="my-3">
            ...
            <div v-for="label in store.AllLabelsIncome" :key="label.id" class="my-2 wrap-break-word">
                <input type="checkbox" v-model="store.selectedLabelsIncome" :value="label" :class="store.isDarkMode == true ? 'accent' : '' "/>
                {{ label.description }}
            </div>

            <div @click="store.LabelModal = true" class="mt-2 border border-gray-400 w-fit rounded py-2 px-3 cursor-pointer">
